...
int main()
{
	...
	CuerpoCoche = Model();
	CuerpoCoche.LoadModel("Models/CuerpoJunto.dae");
	Llanta_FI = Model();
	...
	while (!mainWindow.getShouldClose())
	{
		...
		//infomación al shader de fuentes de iluminación 2
		shaderList[2].SetDirectionalLight(&mainLight);
		shaderList[2].SetPointLights(pointLights, pointLightCount);
		shaderList[2].SetSpotLights(spotLights, spotLightCount);
		...
		//Instancia del coche //
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(7.0f + mainWindow.getmuevex(), 0.2f, 0.0f));	
		modelaux = model;
		model = glm::scale(model, glm::vec3(1.0f, 1.0f, 1.0f));
		//model = glm::rotate(model, -90 * toRadians, glm::vec3(0.0f, 1.0f, 0.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		CuerpoCoche.RenderModel();

		//Llanta delantera izquierda
		model = modelaux;
		model = glm::translate(model, glm::vec3(-1.35f, -0.66f, 1.05f));
		//model = glm::rotate(model, -90 * toRadians, glm::vec3(0.0f, 1.0f, 0.0f));
		model = glm::scale(model, glm::vec3(1.0f, 1.0f, 1.0f));
